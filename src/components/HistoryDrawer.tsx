...
export function HistoryDrawer({ isOpen, onClose, onLoadSession, onNewSession }: Props) {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <header className="h-14 px-4 flex items-center justify-between border-b border-border/50 shrink-0">
          {selectedSession ? (
            <>
              <button
                onClick={handleBack}
                        ...
                        >
                          ...
                          {deleteConfirm === session.id ? (
                            ...
                            <button
                              ...
                              className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-md opacity-0 group-hover:opacity-100 hover:bg-red-500/10 text-muted-foreground hover:text-red-400 border border-transparent hover:border-red-500/20 transition-all duration-200"
                            >
