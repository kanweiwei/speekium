# 🚀 Speekium 守护进程模式

守护进程模式让 Speekium 实现**闪电般的响应速度** ⚡

## 📊 性能对比

| 模式 | 首次响应 | 后续响应 | 内存占用 | 优势 |
|------|---------|---------|---------|------|
| **旧模式** | ~3.5s | ~3.5s | 动态 | 简单 |
| **守护进程** | ~3.5s | **~0.2s** | ~500MB | **快 18 倍！** |

## 🎯 什么是守护进程？

守护进程 = 后台持续运行的程序

**工作原理**：
```
启动时：加载一次所有模型（VAD + ASR + LLM）
使用时：模型已就绪，立刻响应 ⚡
退出时：自动清理，不留痕迹
```

## 🏗️ 架构

```
┌─────────────────────────────────────────┐
│         Tauri Desktop App (Rust)        │
│  ┌─────────┐  ┌──────────┐  ┌────────┐ │
│  │  UI     │  │ Shortcuts │  │  Tray  │ │
│  │ (React) │  │  Handler  │  │  Icon  │ │
│  └────┬────┘  └─────┬────┘  └───┬────┘ │
│       │             │            │      │
│       └─────────────┴────────────┘      │
│                     │                   │
│          ┌──────────▼──────────┐        │
│          │  Daemon Manager     │        │
│          │  (stdin/stdout IPC) │        │
│          └──────────┬──────────┘        │
└─────────────────────┼───────────────────┘
                      │ JSON 命令
        ┌─────────────▼─────────────┐
        │   Python Daemon Process    │
        │                            │
        │  ┌──────────────────────┐  │
        │  │  预加载模型（常驻）   │  │
        │  ├──────────────────────┤  │
        │  │ • VAD (Silero)       │  │
        │  │ • ASR (SenseVoice)   │  │
        │  │ • LLM (Ollama/Claude)│  │
        │  └──────────────────────┘  │
        │                            │
        │  ┌──────────────────────┐  │
        │  │  命令处理器           │  │
        │  │ • record            │  │
        │  │ • chat              │  │
        │  │ • tts               │  │
        │  │ • health            │  │
        │  └──────────────────────┘  │
        └────────────────────────────┘
```

## 📂 新增文件

```
speekium/
├── worker_daemon.py         # 守护进程主程序 ⭐ 新增
├── test_daemon.py          # 守护进程测试脚本 ⭐ 新增
├── start-tauri.sh          # 简化启动脚本 ⭐ 新增
└── the main project/
    └── src-tauri/
        └── src/
            └── lib.rs      # 重写，支持守护进程 ✅ 已更新
```

## 🚀 快速开始

### 方式 1：一键启动（推荐）

```bash
./start-tauri.sh
```

### 方式 2：手动启动

```bash
# 1. 激活 Python 环境
source .venv/bin/activate

# 2. 进入 Tauri 目录
cd the main project

# 3. 启动应用
npm run tauri dev
```

## 🧪 测试守护进程

独立测试 Python 守护进程：

```bash
python3 test_daemon.py
```

**测试内容**：
1. 启动守护进程
2. 健康检查
3. 配置加载
4. TTS 生成
5. LLM 对话
6. 性能测试（10次健康检查）
7. 退出命令

**预期结果**：
```
✅ 所有测试通过！
📊 平均响应时间: ~50ms
⚡ 最快: ~20ms
🐌 最慢: ~100ms
```

## 🔍 工作流程详解

### 1. 应用启动

```rust
// Tauri setup()
fn setup() {
    // 启动 Python 守护进程
    let mut child = Command::new("python3")
        .arg("worker_daemon.py")
        .arg("daemon")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()?;

    // 等待模型加载（~10秒）
    // 守护进程进入待命状态
}
```

### 2. 用户录音

```rust
// 用户点击"录音"按钮
#[tauri::command]
async fn record_audio(mode: String) -> RecordResult {
    // 向守护进程发送命令
    let request = json!({
        "command": "record",
        "args": {"mode": mode}
    });

    // 写入 stdin
    daemon.stdin.write(request);

    // 从 stdout 读取结果（~0.2s）
    let result = daemon.stdout.readline();

    return result;  // 快！
}
```

### 3. LLM 对话

```rust
#[tauri::command]
async fn chat_llm(text: String) -> ChatResult {
    // 同样通过 stdin/stdout 通信
    // 守护进程已加载 LLM，立刻响应
    call_daemon("chat", {"text": text})
}
```

### 4. 应用退出

```rust
// Tauri on_exit()
fn cleanup_daemon() {
    // 发送退出命令
    daemon.send_command("exit", {});

    // 等待进程正常退出
    daemon.process.wait();
}
```

## 🛡️ 健康检查机制

### Rust 端

```rust
fn ensure_daemon_running() {
    if daemon.health_check() {
        return;  // 健康，继续使用
    }

    // 不健康，重启守护进程
    daemon.kill();
    daemon = PythonDaemon::new();
}
```

### 前端自动检查

```typescript
// 每 30 秒检查一次
setInterval(() => {
    invoke('daemon_health')
}, 30000);
```

### Python 守护进程响应

```python
def handle_health(self):
    return {
        "success": True,
        "status": "healthy",
        "command_count": 123,
        "models_loaded": {
            "vad": True,
            "asr": True,
            "llm": True
        }
    }
```

## 📊 性能优化点

### 1. 模型预加载

**旧模式**：每次都加载
```python
def record():
    vad = load_vad()      # 1s
    asr = load_asr()      # 1s
    llm = load_llm()      # 0.5s
    # 处理...
```

**守护进程**：只加载一次
```python
class Daemon:
    def __init__(self):
        self.vad = load_vad()  # 启动时加载
        self.asr = load_asr()
        self.llm = load_llm()

    def record(self):
        # 直接使用，无需加载！
        self.vad.detect(audio)
```

### 2. 进程复用

**旧模式**：启动/关闭开销
```
启动 Python 进程: ~500ms
导入模块: ~500ms
加载模型: ~2500ms
处理请求: ~200ms
关闭进程: ~100ms
---
总计: ~3800ms
```

**守护进程**：只有处理开销
```
处理请求: ~200ms
---
总计: ~200ms (快 19 倍！)
```

### 3. stdin/stdout IPC

**优势**：
- 零依赖（操作系统原生支持）
- 低延迟（~1ms）
- 简单可靠

**通信格式**：
```json
// 请求 (Rust → Python via stdin)
{"command": "chat", "args": {"text": "hello"}}

// 响应 (Python → Rust via stdout)
{"success": true, "content": "你好！"}
```

## ⚙️ 配置

所有配置在 `speekium.py` 中：

```python
# LLM 后端
LLM_BACKEND = "ollama"  # "claude" 或 "ollama"
OLLAMA_MODEL = "qwen2.5:1.5b"

# TTS 后端
TTS_BACKEND = "edge"  # "edge" 或 "piper"

# VAD 参数
VAD_THRESHOLD = 0.7  # 语音检测阈值
```

## 🐛 故障排查

### 问题 1：守护进程启动失败

```bash
# 检查 Python 环境
source .venv/bin/activate
python3 worker_daemon.py daemon

# 应该看到：
# [Daemon] 🚀 Speekium Daemon 初始化中...
# [Daemon] ✅ 所有模型加载完成
```

### 问题 2：健康检查失败

```bash
# 测试守护进程
python3 test_daemon.py

# 查看输出，定位问题
```

### 问题 3：响应慢

**可能原因**：
- Python 环境未激活
- 模型加载失败
- LLM 后端（Ollama）未启动

**检查方法**：
```bash
# 查看守护进程日志
# (Tauri 启动时会输出到终端)

# 确认 Ollama 运行
curl http://localhost:11434

# 测试单独的守护进程性能
python3 test_daemon.py
```

## 📈 后续优化

### 阶段 2：流式响应（规划中）

```rust
// LLM 流式输出
async fn chat_stream(text: String) {
    // 守护进程返回流式数据
    while let Some(chunk) = daemon.read_chunk() {
        window.emit("chat-chunk", chunk);
    }
}
```

### 阶段 3：WebSocket 模式（可选）

更适合高并发场景，但增加复杂度。

## 📝 总结

守护进程模式带来的改进：

✅ **性能提升 18 倍**（后续响应从 3.5s → 0.2s）
✅ **用户体验更流畅**（几乎无等待）
✅ **自动健康检查**（崩溃自动重启）
✅ **优雅退出**（资源清理完整）
✅ **简单可靠**（stdin/stdout IPC）

---

**享受闪电般的 Speekium 体验！** ⚡
